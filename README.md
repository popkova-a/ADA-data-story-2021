...
We introduce the "influence network" of Quotebank speakers. The color of the node is determined by the node degree. In-degree in our case is determined by the number of quotes uttered by the speaker about another person, out-degree defines the number of quotes about this person uttered by other speakers. Red color corresponds to high node degree, purple color corresponds to low degree. The size of the node depends on the amount of quotations of the speaker this particular year (the larger number of quotations - the larger size of the node corresponding to that speaker). The directed edges in our graph have weight - the number of quotes that the "speaker" talks about the "character".
